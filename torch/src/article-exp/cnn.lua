...
cnn:add(nn.SpatialConvolutionMM(1, 28, 5, 5))
cnn:add(nn.SpatialMaxPooling(3, 3, 3, 3))

cnn:add(nn.SpatialConvolutionMM(28, 56, 5, 5))
cnn:add(nn.SpatialMaxPooling(2, 2, 2, 2))

cnn:add(nn.Reshape(56*2*2))
cnn:add(nn.Linear(56*2*2, 200))
cnn:add(nn.Tanh())
...
local w,dE_dw = cnn:getParameters()
...
local function train()
    ...
    for t = 1, trainSize, batchSize do
        ...
        local eval_E = function(w)
            ...
            local y = cnn:forward(x)
            local E = loss:forward(y,yt)

            -- estimate df/dW
            local dE_dy = loss:backward(y,yt)
            cnn:backward(x,dE_dy)
...
for t = 1,testSize do
    ...
    -- test sample
    local pred = cnn:forward(input):view(10)
...
-- save cnn
path_to_save = "/home/dmitry/Projects/DNN-develop/torch/src/article-exp/cnn.net"
torch.save(path_to_save, cnn)
